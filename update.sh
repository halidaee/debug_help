...
# Uncomment line below to install dependent packages, including nuclear_ard from GitHub. 
singularity exec --cleanenv --env R_LIBS_USER=${my_sif_r_packages} ${my_sif} Rscript --no-save --no-restore --verbose -e Rscript --no-save --no-restore --verbose install_packages.R
...
array=(600 603 605 716)

# Execute proper_cross_validation_debug.R for each array index. 
for i in "${array[@]}"
do
    singularity exec --cleanenv --env OMP_NUM_THREADS=${OMP_NUM_THREADS},MKL_NUM_THREADS=${MKL_NUM_THREADS},MKL_THREADING_LAYER=${MKL_THREADING_LAYER},R_LIBS_USER=${my_sif_r_packages},SLURM_ARRAY_TASK_ID=${i} ${my_sif} Rscript --no-save --no-restore --verbose proper_cross_validation_debug.R > job_array_output_${i}_debug.log 2>&1
done